...
def display_board(board, switch): #switch = 1 for win 0 for in-game
    for i in range(1, 4):
        print(color.BOLD + color.GREEN + f"{board[i]}" + color.END, end=" | ")
    print()
    print("___________")
    for i in range(4, 7):
        print(color.BOLD + color.GREEN + f"{board[i]}" + color.END, end=" | ")
...
def game_on():
    score = [0,0]
    tie = 0
    print(color.BOLD+"Welcome to TIC TAC TOE"+color.END)
    while True:
